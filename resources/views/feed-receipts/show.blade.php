    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">{{ thai_date($feedReceipt->receipt_date) }} | อาหาร {{ $feedReceipt->feed_code }} | เลขที่ผลิต {{ $feedReceipt->production_lot }}</p>
            </div>
        ...
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-4 md:grid-cols-4">
                <div class="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <div class="mt-1 font-semibold text-slate-900">{{ $feedReceipt->farm->farm_name }}</div>
                </div>
                <div class="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
                    <div class="text-sm text-slate-500">วันที่รับ</div>
                    <div class="mt-1 font-semibold text-slate-900">{{ thai_date($feedReceipt->receipt_date) }}</div>
                </div>
                <div class="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
                    <div class="text-sm text-slate-500">เบอร์อาหาร</div>
                    <div class="mt-1 font-semibold text-slate-900">{{ $feedReceipt->feed_code }}</div>
                </div>
                <div class="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
                    <div class="text-sm text-slate-500">รวมทั้งหมด</div>
                    <div class="mt-1 font-semibold text-slate-900">{{ number_format((float) $feedReceipt->items->sum('quantity_kg'), 2) }} กก.</div>
                </div>
            </div>

            <div class="mt-6 overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-4 py-3">
                    <h2 class="text-base font-semibold text-slate-900">รายการลงเล้า</h2>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white">
                            @foreach ($feedReceipt->items->sortBy('house.house_no') as $item)
                                <tr>
                                    <td class="px-4 py-3 font-medium text-slate-900">เล้า {{ $item->house->house_no }}</td>
                                    <td class="px-4 py-3 text-slate-700">{{ $item->house->house_name ?: '-' }}</td>
                                    <td class="px-4 py-3 text-right font-semibold text-slate-900">{{ number_format((float) $item->quantity_kg, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            <div class="mt-6 rounded-md border border-slate-200 bg-white p-4 shadow-sm">
