<x-app-layout>
    <x-slot name="header">
        <div>
            <h1 class="text-xl font-semibold text-slate-900">ราคาขาย</h1>
            <p class="mt-1 text-sm text-slate-600">กำหนดราคาขายไก่เนื้อแบบราคากลาง บาท/กก. ใช้ร่วมกันทุกฟาร์ม</p>
        </div>
    </x-slot>
        ...
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            ...
            @if (auth()->user()->isSuperAdmin())
                <div class="mb-6 rounded-md border border-slate-200 bg-white p-5 shadow-sm">
                    <form method="POST" action="{{ route('sale-price-masters.store') }}">
                        @csrf
                        <div class="grid gap-4 md:grid-cols-[1fr_1fr_auto_auto] md:items-end">
                            <div>
                                <x-input-label for="price_per_kg" value="ราคาขาย/กก." />
                                <x-text-input id="price_per_kg" name="price_per_kg" type="number" min="0" step="0.01" class="mt-1 block w-full text-right" :value="old('price_per_kg')" required />
            ...
            @else
                <div class="mb-6 rounded-md border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-800">
                    ราคาขายเป็นราคากลางของระบบ หากต้องการแก้ไขให้ติดต่อ Super Admin
                </div>
            @endif
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white">
                            @forelse ($prices as $row)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-right font-semibold text-slate-900">{{ number_format((float) $row->price_per_kg, 2) }}</td>
                                    <td class="px-4 py-3">
                                    ...
                                    @if (auth()->user()->isSuperAdmin())
                                        <td class="px-4 py-3 text-right">
                                            <form method="POST" action="{{ route('sale-price-masters.destroy', $row) }}" onsubmit="return confirm('ยืนยันลบราคาขายนี้หรือไม่?')">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ auth()->user()->isSuperAdmin() ? 5 : 4 }}" class="px-4 py-8 text-center text-slate-500">ยังไม่มีข้อมูลราคาขาย</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
